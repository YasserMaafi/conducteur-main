<?php
// This file contains the notification dropdown for client pages
...
$clientNotifications = $notifStmt->fetchAll();
...
$clientNotificationCount = $notifCountStmt->fetch()['count'];
?>
...
<div class="dropdown me-3">
    <button class="btn btn-outline-light position-relative" 
        ...
        <?php if ($clientNotificationCount > 0): ?>
    ...
    <ul class="dropdown-menu dropdown-menu-end shadow-sm notification-dropdown" aria-labelledby="notificationDropdown" style="width: 400px; max-height: 400px; overflow-y: auto;">
        <li>
            <div class="dropdown-header bg-light d-flex justify-content-between align-items-center py-3">
                ...
                <?php if ($clientNotificationCount > 0): ?>
                    <span class="badge bg-primary rounded-pill"><?= $clientNotificationCount ?> nouvelle<?= $clientNotificationCount > 1 ? 's' : '' ?></span>
                <?php endif; ?>
            </div>
        </li>
        ...
        <?php if ($clientNotificationCount > 0): ?>
            <?php foreach ($clientNotifications as $notification): 
                ...
                $link = $metadata['link'] ?? 'notifications.php';
            ?>
                <li>
                    <a class="dropdown-item notification-item py-2 px-3 border-bottom" href="<?= $link ?>">
                        <div class="d-flex">
        ...
        <li>
            ...
            <a class="dropdown-item text-center py-2 bg-light" href="notifications.php">
